...
qTMPS <- function(p = c(2, 5, 10, 20, 25, 50, 100, 200, 500, 1000),
  ...
  Results <- TMPS_model(
    Flood_events = Flood_events, Daily_discharge = Daily_discharge,
...
TMPS_model <- function(Flood_events, Daily_discharge, return_TMPS, Threshold_Q,
  ...
  if (!is.factor(Flood_events$Type)) {
    ...
    warning(paste0(
      "Floodtypes aren't factors, so assuming they are:\n",
  ...
  if (any(min_events_reached)) {
    warning(paste0(
      "Floodtype ", paste(g_const[min_events_reached], collapse = " - "),
  ...
  if (any(any_under_TH)) {
    warning(paste0(
      "Floodtype ", paste(g_const[any_under_TH], collapse = " - "),
  ...
  if (!is.null(p_input)) {
    R <- matrix(NA,
      nrow = length(return_TMPS), ncol = length(p_input),
  ...
  if (!is.null(q_input)) {
    R <- matrix(NA,
      nrow = length(return_TMPS), ncol = length(q_input),
...
hydroyear <- function(Data) {
  if (is.na(Data[, 1])) {
    ...
    if (as.numeric(as.character(format(Data[, 1], "%m"))) > 10) {
      return(as.numeric(as.character(format(Data[, 1], "%Y"))) + 1)
    } else {
...
qPOT_mixmixPOT <- function(Ta, params_POT, TH, params, antseas, AnzT) {
  uniroot_NA(optfun_mixmixPOT,
    Ta = Ta, params_POT = params_POT, TH = TH,
    params = params, antseas = antseas, AnzT = AnzT, interval = c(0, 10000000)
  )$root
}
...
TmixPOT <- function(x, params_POT, TH, params, antseas, AnzT) {
  optim(10, optquan_v2,
    x = x, params_POT = params_POT, TH = TH, params = params,
    antseas = antseas, AnzT = AnzT, method = "Brent", lower = 0, upper = 1000000000
  )$par
}
...
mixPOT2 <- function(x, params_POT, TH, params, antseas, AnzT) {
  ...
  for (i in AnzT) {
    res <- res * (pgev(TH[[i]],
      xi = params[[i]][1], mu = params[[i]][2],
      beta = params[[i]][3]
    ) + (pgpd(x, xi = params_POT[[i]][1], mu = TH[[i]], beta = params_POT[[i]][2])) * (1 - pgev(TH[[i]],
